...
export const useForm = ({ definition, initialValues = {}, onChange }: UseFormProps) => {
  ...
  useEffect(() => {
    ...
    definition.fields.forEach((field) => {
      if (initialValues[field.name] !== undefined) {
        ...
        switch (field.type) {
          case 'boolean': newValues[field.name] = false; break;
          case 'multiselect': newValues[field.name] = []; break;
          case 'number': case 'date': newValues[field.name] = null; break;
          default: newValues[field.name] = '';
        }
      }
    });
    setValues(newValues);
  ...
  const checkFieldVisibility = useCallback((field: FormField, currentValues = values) => {
    if (!field.conditions || field.conditions.length === 0) return true;
    
    return field.conditions.every((condition) => {
      const targetField = definition.fields.find(f => f.id === condition.fieldId);
  ...
  const validateField = useCallback((field: FormField, value: any, currentValues = values): string | null => {
  ...
  const calculateIsValid = useCallback((currentValues: Record<string, any>) => {
