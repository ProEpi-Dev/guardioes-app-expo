...
export const getTrackCycleDetails = async (
  id: number
): Promise<TrackCycle | null> => {
  try {
    const response = (await apiClient(`/v1/track-cycles/${id}`, {
      method: 'GET',
    })) as any;
    return response;
  } catch (error) {
...
export const getTrackProgress = async (
  ...
  return await apiClient(
    `/v1/track-progress/participation/${participationId}/cycle/${cycleId}`,
    { method: 'GET' }
  );
};

export const startTrackProgress = async (
  participationId: number,
  trackCycleId: number
) => {
...
export const updateSequenceProgress = async (
  trackProgressId: number,
  sequenceId: number,
  data: { status?: string; timeSpentSeconds?: number; visits_count?: number }
) => {
  return await apiClient(
    `/v1/track-progress/${trackProgressId}/sequence/${sequenceId}`,
    {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
export const completeContentSequence = async (
  trackProgressId: number,
  sequenceId: number
) => {
  return await apiClient(
    `/v1/track-progress/${trackProgressId}/sequence/${sequenceId}/complete-content`,
    {
      method: 'POST',
    }
  );
...
export const completeQuizSequence = async (
  trackProgressId: number,
  sequenceId: number,
  quizSubmissionId: number
) => {
  return await apiClient(
    `/v1/track-progress/${trackProgressId}/sequence/${sequenceId}/complete-quiz`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const mergeTrailWithProgress = async (
  trailFullData: any,
  progressData: any,
  submissions: any[] = []
): Promise<Section[]> => {
      ...
      if (isQuiz) {
        try {
          ...
          passingScore =
            ...
            quizDetails?.latestVersion?.passingScore ??
            null;
          maxAttempts =
            quizDetails?.maxAttempts ??
            quizDetails?.latestVersion?.maxAttempts ??
            null;
          timeLimitMinutes =
            quizDetails?.timeLimitMinutes ??
            quizDetails?.latestVersion?.timeLimitMinutes ??
            null;
        } catch (err) {
...
export const checkMandatoryCompliance = async (
  participationId: number
): Promise<{ is_compliant: boolean; mandatory_slug?: string }> => {
  try {
    const response = (await apiClient(
      `/v1/track-progress/mandatory-compliance?participationId=${participationId}`,
      {
        method: 'GET',
      }
    )) as any;
