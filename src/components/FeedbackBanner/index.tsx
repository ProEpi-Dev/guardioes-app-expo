...
export const FeedbackBanner: React.FC<FeedbackBannerProps> = ({
  ...
  const selectedOption = question.options?.find(
    (opt: any) => normalize(opt.value) === normalize(userAnswer)
  );
    ...
    return (
      <View style={[styles.banner, { backgroundColor: '#E3F2FD' }]}>
        <View style={styles.header}>
          ...
          <Text style={[styles.title, { color: '#1565C0' }]}>
            Resposta Registrada
          </Text>
        </View>
        {feedbackText && (
          <Text style={[styles.text, { color: '#1565C0' }]}>
            {feedbackText}
          </Text>
        )}
      </View>
  ...
  return (
    <View style={[styles.banner, { backgroundColor: bg, borderColor: color }]}>
      ...
      {feedbackText && (
        <Text style={[styles.text, { color }]}>{feedbackText}</Text>
      )}
      {hasCorrectAnswer && (
